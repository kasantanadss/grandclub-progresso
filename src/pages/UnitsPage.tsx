...
const UnitsPage = () => {
  ...
  const handleSave = () => {
    if (!form.apartment || !form.ownerName) return;
    if (editId) {
      updateUnit(editId, form);
      setEditId(null);
    } else {
      addUnit({ ...form, id: crypto.randomUUID() });
    }
    setForm(emptyUnit());
    setShowForm(false);
  };
  ...
  const filteredUnits = filter === 'all' ? units : units.filter((u) => u.financialStatus === filter);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Button onClick={() => { setShowForm(true); setEditId(null); setForm(emptyUnit()); }}>
      ...
      <div className="flex gap-2 flex-wrap">
        {(['all', 'adimplente', 'acordo', 'inadimplente'] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
              filter === f ? 'gradient-navy text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted/80'
            }`}
          >
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <Input value={form.apartment} onChange={(e) => setForm({ ...form, apartment: e.target.value })} placeholder="Ex: 101" />
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">Proprietário</label>
                <Input value={form.ownerName} onChange={(e) => setForm({ ...form, ownerName: e.target.value })} placeholder="Nome completo" />
              </div>
              <div>
              ...
              <div>
                <label className="text-sm font-medium mb-1 block">Nº de Vagas</label>
                <select
                  className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm"
            ...
            <div className="flex flex-wrap gap-4 mt-4">
              {[
                ...
                { key: 'isPCD' as const, label: 'PCD / Locomoção' },
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/50">
                ...
                <th className="text-center p-3 font-medium">PCD</th>
            ...
            <tbody>
              {filteredUnits.map((unit) => (
                <tr key={unit.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="p-3 text-center">{unit.numberOfSpots}{unit.rentsSecondSpot ? ' (+1)' : ''}</td>
                  ...
                  <td className="p-3 text-center">{unit.isPCD ? '♿' : '—'}</td>
                  <td className="p-3 text-right">
                    <div className="flex justify-end gap-1">
                      <button onClick={() => handleEdit(unit)} className="p-2 rounded-lg hover:bg-muted transition-colors">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => removeUnit(unit.id)} className="p-2 rounded-lg hover:bg-destructive/10 text-destructive transition-colors">
              ...
              {filteredUnits.length === 0 && (
                <tr><td colSpan={7} className="p-8 text-center text-muted-foreground">Nenhuma unidade cadastrada</td></tr>
              )}
            </tbody>
          </table>
        </div>
